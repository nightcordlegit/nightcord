...
function YouCordTabIcon() {
    return <img src={YOUCORD_TAB_ICON} alt="YouCord" style={{ width: 18, height: 18, borderRadius: 4 }} />;
}
...
const makeCategoryOptions = (othersCount?: number) => [
    ...
    { label: "YouCord", value: SearchStatus.YOUCORD },
...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of YouCord"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.YOUCORD:
                if (!pluginMeta?.folderName?.startsWith("src/youcordplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (pluginMeta?.folderName?.startsWith("src/youcordplugins/") || pluginMeta?.folderName?.startsWith("src/plugins/_")) return false;
    ...
    // Only filter/categorize plugin DATA here â€” no JSX created yet
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                ...
                if (folderName.startsWith("src/youcordplugins/")) {
                    youcordData.push(p);
                } else {
                    othersData.push(p);
                }
        ...
        return { youcordData, othersData, requiredData };
    }, [sortedPlugins, searchValue, newPluginsSet, depMap, pluginFilter]);

    const allDataLength = youcordData.length + othersData.length;
    ...
    const youcordPlugins = youcordVisibleData.map(makeCard);
    ...
    const categoryStats = useMemo(() => {
        ...
        if (searchValue.status === SearchStatus.YOUCORD) {
            ...
            plugins = plugins.filter(p => !isYouCordPlugin(p));
        ...
        <SettingsTab>
                ...
                {!premiumOnly && (
                    <div className={cl("ecosystem-banner")}>
                        <div className={cl("ecosystem-banner-text")}>
                            ...
                            <Paragraph>{t("Manage your YouCord and community plugins here. Enable, disable, and configure them to your liking.")}</Paragraph>
                        </div>
                            ...
                            onClick={() => {
                                ...
                                if (native?.process?.env) {
                                    ...
                                    if (home) {
                                        ...
                                        const folderPath = isWindows ? `${home}\\Documents\\YouCord\\userplugins` : `${home}/Documents/YouCord/userplugins`;
                ...
                <div className={classes(Margins.bottom20, cl("filter-controls"))}>
                    ...
                    <div className={cl("filter-buttons")} style={{ minWidth: 220 }}>
                        <SearchableSelect
                            ...
                            value={makeCategoryOptions(totalOtherPlugins).find(o => o.value === searchValue.status)?.value ?? SearchStatus.YOUCORD}
                            onChange={(v: any) => {
                            ...
                            renderOptionPrefix={(o: any) => {
                                if (o?.value === SearchStatus.YOUCORD) return <YouCordTabIcon />;
            ...
            {premiumOnly ? (
                <>
                    ...
                    {youcordData.length || othersData.length
                        ? (
                            <div className={cl("grid")}>
                                {[...youcordPlugins, ...othersVisible].length
                                    ? [...youcordPlugins, ...othersVisible]
                ...
                <>
                    {youcordData.length > 0 && searchValue.status === SearchStatus.YOUCORD && (
                        <div className={cl("grid")}>
                            {youcordPlugins}
                        </div>
                    )}
                    ...
                    {(searchValue.status !== SearchStatus.YOUCORD && searchValue.status !== SearchStatus.OTHERS && searchValue.status !== SearchStatus.USER_PLUGINS) && (
                        <div className={cl("grid")}>
                            {youcordPlugins}
                            {othersVisible}
                        </div>
                    ...
                    {youcordData.length === 0 && othersData.length === 0 && searchValue.status !== SearchStatus.USER_PLUGINS && (
                        <ExcludedPluginsList search={search} />
                    )}
