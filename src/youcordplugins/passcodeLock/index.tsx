...
// through the cracks: window.__youcordPCLReset()
...
function lock(button: HTMLElement | null = document.body) {
    ...
    try {
        ...
        showToast("PasscodeLock failed to open â€” check the console for details.", Toasts.Type.FAILURE);
    }
}
...
export default definePlugin({
    ...
    authors: [{ name: "arg0NNY (ported by YouCord)", id: 0n }],
    settings,
    ...
    async start() {
        ...
        (window as any).__youcordPCLReset = () => forceReset("manually triggered via window.__youcordPCLReset()");
    ...
    stop() {
        ...
        clearTimeout(autolockTimeout);
        clearTimeout(watchdogTimeout);
        delete (window as any).__youcordPCLReset;
