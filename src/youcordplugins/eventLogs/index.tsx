...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("youcord_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("youcord_logs", JSON.stringify(toSave));
    } catch { }
}
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("YouCord Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `youcord_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("youcord-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
