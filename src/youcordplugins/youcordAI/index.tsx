...
function findFriend(name: string): { id: string; username: string; } | null {
    ...
    } catch (e) { console.warn("[YouCordAI] findFriend:", e); }
    return null;
}
...
function joinVoiceChannel(name: string): void {
    ...
    } catch (e) { console.warn("[YouCordAI] joinVoiceChannel guild search:", e); }
                ...
                <div className="nai-header-left">
                    ...
                    <div className="nai-header-info">
                        <div className="nai-header-title-row">
                            <span className="nai-header-title">{t("YouCord AI")}</span>
                    ...
                    {messages.length === 0 ? (
                        <div className="nai-empty">
                            ...
                            <p className="nai-empty-sub">
                                {hasKey ? t("Ask anything!") : t("Configure your API key in YouCord Settings > Plugins > YouCordAI")}
                            </p>
                                ...
                                <div className="nai-msg-body">
                                    {!grouped && (
                                        <div className="nai-msg-meta">
                                            <span className="nai-msg-author">{msg.role === "user" ? (UserStore.getCurrentUser()?.globalName ?? UserStore.getCurrentUser()?.username ?? "You") : "YouCord AI"}</span>
...
function YouCordAINavButton({ selected }: { selected?: boolean; }) {
    ...
    return (
        <div className={`nai-nav-item ${selected ? "selected" : ""}`} role="button" tabIndex={0}
            ...
            <span className="nai-nav-label">{t("YouCord AI")}</span>
...
export default definePlugin({
    ...
    patches: [
        {
            // Patch 1 : Remplace la page Boutique (Shop) par notre panneau YouCordAI
    ...
    start() {
        ...
        const inject = () => {
            ...
            if (createRoot) {
                ...
                this._reactRoot.render(<YouCordAINavButton />);
            } else {
    ...
    renderNavButton(selected?: boolean) {
        return <YouCordAINavButton selected={selected} />;
    },

    renderPanel() {
        return <YouCordAIPanel />;
    },
    ...
    toolboxActions: {
        "YouCord AI"() {
            openModal(props => <YouCordAIChat rootProps={props} />);
        },
    },
});
