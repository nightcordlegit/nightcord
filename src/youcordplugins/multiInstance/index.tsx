...
const MI_TOKEN_CACHE_KEY = "youcord-mi-token-cache";
                ...
                <div className="mi-list">
                    ...
                    {allAccounts.length === 0 ? (
                        ...
                        const tagText = acc.hasToken
                            ? (acc.isNative ? "ðŸ”— Discord Account" : "ðŸ”‘ Token")
                            : t("Switch only");
                        return (
                            <div
                                key={acc.id}
...
function MultiInstanceButton() {
    return (
        <HeaderBarButton
            ...
            tooltip="Multi-instance â€” open Discord with another account"
...
export default definePlugin({
    ...
    async start() {
        ...
        captureCurrentToken();
        addHeaderBarButton("youcord-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("youcord-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("youcord-mi-ctx-root");
        if (root) root.remove();
    },
});
