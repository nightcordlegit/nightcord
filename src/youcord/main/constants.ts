...
const YouCordDir = dirname(process.execPath);

export const PORTABLE =
    process.platform === "win32" &&
    !process.execPath.toLowerCase().endsWith("electron.exe") &&
    !existsSync(join(YouCordDir, "Uninstall YouCord.exe"));

export const DATA_DIR =
    process.env.YouCord_USER_DATA_DIR || (PORTABLE ? join(YouCordDir, "Data") : join(app.getPath("userData")));
...
export const USER_AGENT = `YouCord/${app.getVersion()} (https://git.${domain}/youcord/youcord)`;
