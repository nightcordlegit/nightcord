...
const options = {
    ...
    repair: {
        type: "boolean",
        short: "r",
        description: "Re-download YouCord and restart"
    }
...
const extraOptions = {
    ...
    "ozone-platform": {
        hidden: process.platform !== "linux",
        type: "string",
        description: "Whether to run YouCord in Wayland or X11 (XWayland)",
        options: ["x11", "wayland"]
    }
} satisfies Record<string, Option>;
...
export async function checkCommandLineForRepair() {
    ...
    if (State.store.YouCordDir) {
        console.error("Cannot repair: using custom YouCord directory. Remove it in settings first.");
        process.exit(1);
    }

    console.log("Repairing YouCord...");
...
function checkCommandLineForToggleCommands() {
    ...
    console.error("YouCord is not running. Toggle commands require a running instance.");
    app.exit(1);
}
...
function checkForSecondInstance() {
    ...
    if (!app.requestSingleInstanceLock({ IS_DEV })) {
        if (IS_DEV) {
            console.log("YouCord is already running. Quitting previous instance...");
            return;
        } else {
            console.log("YouCord is already running. Quitting...");
            app.exit(0);
        }
    }
