...
handleSync(IpcEvents.GET_GIT_HASH, () => YouCord_GIT_HASH);
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.YouCordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { YouCordDir } = State.store;
    if (!YouCordDir) return;

    const stats = await stat(YouCordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(YouCordDir);
});
