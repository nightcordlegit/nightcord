...
async function translateBatch(strings, targetLang) {
    ...
    try {
        ...
        const parts = fullTranslated.split(/\s*[\[âŸ¦]SEP[\]âŸ§]\s*/i);
        ...
        console.warn(`  âš  Split mismatch (got ${parts.length}, expected ${strings.length}) for ${targetLang}, retrying one-by-one...`);
        return null; // signal to retry individually
    } catch (e) {
        console.error(`  âœ— Error translating to ${targetLang}: ${e.message}`);
        return null;
    }
}
...
async function main() {
    ...
    console.log(`ðŸ“¦ ${plugins.length} plugins, ${allDescriptions.length} unique descriptions`);
    ...
    console.log(`ðŸ”„ Need to translate ${uncachedDescriptions.length} strings to 4 languages`);
    ...
    for (let li = 0; li < LANGUAGES.length; li++) {
        ...
        if (needTranslation.length === 0) {
            console.log(`âœ… ${lang}: already cached`);
            continue;
        }

        console.log(`\nðŸŒ Translating ${needTranslation.length} strings to ${lang}...`);
    ...
    console.log("\nðŸ“ Generating pluginI18n.ts...");

    const lines = [
        `/*`,
        ` * YouCord - Auto-generated plugin description translations`,
